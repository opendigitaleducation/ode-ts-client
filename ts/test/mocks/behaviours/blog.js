...
/***/ (function(module, exports, __webpack_require__) {
	...
	var slugify = function (string) {
	    if (!string)
	        return "";
	    var a = 'Ã Ã¡Ã¤Ã¢Ã£Ã¥ÄƒÃ¦Ã§Ã¨Ã©Ã«ÃªÇµá¸§Ã¬Ã­Ã¯Ã®á¸¿Å„Ç¹Ã±Ã²Ã³Ã¶Ã´Å“Ã¸á¹•Å•ÃŸÅ›È™È›Ã¹ÃºÃ¼Ã»Ç˜áºƒáºÃ¿ÅºÂ·/_,:;';
	...
	exports.blogModel = {
	    ...
	    Blog: function (data) {
	        ...
	        this.collection(entcore_1.Behaviours.applicationsBehaviours.blog.model.Post, {
	            syncPosts: function (cb, paginate, search, filters, publicPost) {
	                ...
	                var postUrl = publicPost ? '/blog/pub/posts/' : '/blog/post/list/all/';
	                entcore_1.http().get(postUrl + that._id, jsonParam).done(function (posts) {
	                    if (posts.length > 0) {
	                        ...
	                        this.all.forEach(function (post) {
	                            posts = entcore_1._.reject(posts, function (p) { return p._id === post._id; });
	                        });
	                        this.addRange(posts);
	                        this.page++;
	                    }
	            ...
	            syncAllPosts: function (cb, publicPost) {
	                ...
	                this.postsLoading = true;
	                var postUrl = publicPost ? '/blog/pub/posts/' : '/blog/post/list/all/';
	                entcore_1.http().get(postUrl + that._id).done(function (posts) {
	                    posts.map(function (item) {
	                        item.blogId = data._id;
	            ...
	            syncOnePost: function (cb, id, publicPost) {
	                if (publicPost === void 0) { publicPost = false; }
	                var postUrl = publicPost ? '/blog/pub/posts/' : '/blog/post/list/all/';
	                entcore_1.http().get(postUrl + that._id, { postId: id }).done(function (posts) {
	                    if (posts.length > 0) {
	                        var type = this.model.data['publish-type'];
	                        posts.map(function (item) {
	                            item.blogId = data._id;
	            ...
	            addDraft: function (post, callback) {
	                entcore_1.http().postJson('/blog/post/' + that._id, post).done(function (result) {
	                    post._id = result._id;
	                    this.push(post);
	    ...
	    App: function () {
	        this.collection(entcore_1.Behaviours.applicationsBehaviours.blog.model.Blog, {
	            syncPag: function (cb, paginate, search) {
	                ...
	                entcore_1.http().get('/blog/list/all', { page: this.page, search: search }).done(function (blogs) {
	                    if (blogs.length > 0) {
	            ...
	            syncAll: function (cb) {
	                ...
	                this.blogsLoading = true;
	                entcore_1.http().get('/blog/list/all').done(function (blogs) {
	                    _this.load(blogs);
	                    _this.blogsLoading = false;
	                    _this.trigger('sync');
	                    if (typeof cb === "function") {
	            ...
	            counterPost: function (blogId, cb) {
	                entcore_1.http().get('/blog/counter/' + blogId).done(function (obj) {
	                    if (typeof cb === "function") {
	                        cb(obj);
	                    }
	                });
	    ...
	    register: function () {
	        ...
	        this.Blog.prototype.create = function (fn) {
	            entcore_1.http().postJson('/blog', this)
	        ...
	        this.Blog.prototype.saveModifications = function (fn) {
	            entcore_1.http().putJson('/blog/' + this._id, this).done(function () {
	                if (typeof fn === 'function') {
	                    fn();
	                }
	            });
	        ...
	        this.Post.prototype.open = function (cb) {
	            entcore_1.http().get('/blog/post/' + this.blogId + '/' + this._id, { state: this.state }).done(function (data) {
	        ...
	        this.Post.prototype.submit = function (callback) {
	            ...
	            entcore_1.http().putJson('/blog/post/submit/' + this.blogId + '/' + this._id).done(function () {
	                if (typeof callback === 'function') {
	                    callback(true);
	                }
	                this.trigger('change');
	        ...
	        this.Post.prototype.publish = function (callback, selfPost) {
	            ...
	            entcore_1.http().putJson('/blog/post/publish/' + this.blogId + '/' + this._id).done(function () {
	                if (typeof callback === 'function') {
	                    callback(true);
	                    this.trigger('change');
	                }
	        ...
	        this.Post.prototype.create = function (callback, blog, state) {
	            entcore_1.http().postJson('/blog/post/' + blog._id, {
	        ...
	        this.Post.prototype.saveModifications = function (callback) {
	            entcore_1.http().putJson('/blog/post/' + this.blogId + '/' + this._id, {
	                content: this.content,
	                title: this.title
	            }).done(function (rep) {
	                if (typeof callback === 'function') {
	        ...
	        this.Post.prototype.remove = function (callback) {
	            entcore_1.http().delete('/blog/post/' + this.blogId + '/' + this._id)
	                .done(function () {
	                if (typeof callback === 'function') {
	                    callback();
	                }
	        ...
	        this.Post.prototype.comment = function (comment) {
	            ...
	            return new Promise(function (resolve, reject) {
	                entcore_1.http().postJson('/blog/comment/' + _this.blogId + '/' + _this._id, comment).done(function () {
	                    this.comments.sync();
	                    resolve();
	                }.bind(_this));
	            });
	        ...
	        this.Post.prototype.updateComment = function (comment) {
	            entcore_1.http().putJson('/blog/comment/' + this.blogId + '/' + this._id + '/' + comment.id, comment).done(function () {
	                this.comments.sync();
	            }.bind(this));
	        };
	        this.Blog.prototype.open = function (success, error) {
	            entcore_1.http().get('/blog/' + this._id)
	                .done(function (blog) {
	        ...
	        articles: {
	            ...
	            controller: {
	                init: function () {
	                    ...
	                    var blog = new entcore_1.Behaviours.applicationsBehaviours.blog.model.Blog({ _id: this.source._id });
	                    this.newPost = new entcore_1.Behaviours.applicationsBehaviours.blog.model.Post();
	                    blog.open(function () {
	                ...
	                createBlog: function () {
	                    ...
	                    if (this.snipletResource) {
	                        ...
	                        this.blog.title = this.blog.title || 'Les actualitÃ©s du site ' + this.snipletResource.title;
	                    ...
	                    this.blog.save(function () {
	                        ...
	                        var post = {
	                            state: 'SUBMITTED',
	                            content: '<p>Voici le premier billet publiÃ© sur votre site !</p><p>Vous pouvez crÃ©er de nouveaux billets (si vous Ãªtes contributeur) en cliquant sur le bouton "Ajouter un billet" ' +
	                                'ci-dessus, ou en accÃ©dant directement Ã  l\'application Blog. Vos visiteurs pourront Ã©galement suivre vos actualitÃ©s depuis leur application, ' +
	                                'et seront notifiÃ©s lorsque votre site sera mis Ã  jour.</p><p>La navigation, Ã  gauche des billets, est automatiquement mise Ã  jour lorsque vous ajoutez' +
	                                ' des pages Ã  votre site.</p>',
